#!/usr/bin/env node
/**
 * Parses the real gym routine from data/rutina-para-Jorge-sept-2026-144bd5.xlsx
 * using SheetJS (xlsx), already a project dependency, and writes a structured,
 * versioned JSON file consumed by the app at build time.
 *
...
const SOURCE_XLSX = path.join(ROOT, 'data', 'rutina-para-Jorge-sept-2026-144bd5.xlsx')
const OUT_FILE = path.join(ROOT, 'data', 'generated', 'routine.json')

if (!existsSync(SOURCE_XLSX)) {
  console.error(`No se encontró el Excel de la rutina en: ${SOURCE_XLSX}`)
  process.exit(1)
}
...
function parseSetsAndReps(raw) {
  ...
  const targetReps = repsMatch ? repsMatch[1].trim() : null
...
function slugifyExercise(dayNumber, muscleGroup, name, index) {
  const base = `${muscleGroup} ${name}`
    ...
    .replace(/[^a-z0-9]+/g, '-')
    .replace(/(^-|-$)/g, '')
  return `g${dayNumber}-${index}-${base}`.slice(0, 80)
}
...
function main() {
  ...
  for (const row of rows) {
    ...
    currentDay.exercises.push({
      id: slugifyExercise(currentDay.dayNumber, muscleGroupRaw, name, exerciseIndexInDay),
  ...
  if (days.length === 0) {
    console.error('No se detectaron días de rutina en el Excel. Revisa el formato de la hoja.')
    process.exit(1)
  }
  ...
  const output = {
    ...
    sourceFile: path.basename(SOURCE_XLSX),
  ...
  console.log(`OK: ${days.length} días, ${totalExercises} ejercicios -> ${path.relative(ROOT, OUT_FILE)}`)
}
